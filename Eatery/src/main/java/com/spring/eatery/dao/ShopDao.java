...
public class ShopDao {
	...
	public List<Map<String, Object>> findAll(Map<String, Object> map) {
		...
		sql.append("	*cos(radians(LAT))*cos(radians(LNG)"); // 그냥 join 하면 사진 꼭 있는 경우만 매장 등록됨, outer join 해서 사진 없어도  등록케 함(해골 이미지)
		sql.append("  -radians(?))+sin(radians(?))");
		sql.append("	*sin(radians(LAT)))) AS distance");
		...
		return jt.queryForList(
				sql.toString(), map.get("lat"),
				map.get("lng"),map.get("lat"),map.get("distance"));
