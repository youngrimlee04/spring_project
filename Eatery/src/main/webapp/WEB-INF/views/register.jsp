...
<script type="text/javascript"
	src="//dapi.kakao.com/v2/maps/sdk.js?appkey=9df5e6336b39ea13f43e62b7d708f08b&libraries=services"></script>
<script> 
	$('[type=submit]').click(function() { // 위에 있는 submit 버튼 불러옴
	// 등록 버튼 누른 후 메세지 출력하고 현재 화면에 머무름
		var form = $("form")[0];
		var formData = new FormData(form);
		$.ajax({
			contentType : false,
			processData : false,
			url : "register", // 만들어진 Formdata 전송
			type : "post",
			data : formData,
			success : function(res) {
				alert(res.msg); // 등록완료 메세지 출력
				location ="shop"; // 매장 등록 완료 후 shop.jsp
			}
		});
		return false; //submit 버튼 기본 기능 무효화
	});
	// TODO 현재 위도 경도 가져오기
	...
	navigator.geolocation
			.getCurrentPosition(function(pos) {
				...
				document.querySelector("#location").textContent = "현재 위치는"
						+ latitude + "," + longitude;
